...
async def get_template(template_id: str) -> ScenarioTemplate | None:
    db = get_database()
...
async def get_template_personas(template_id: str) -> list[Stakeholder]:
    ...
    if not template:
        return []
    personas: list[Stakeholder] = []
    for pid in template.suggested_persona_ids:
        p = await db.get_stakeholder(pid)
        if p:
            personas.append(p)
    return personas
