                ...
                return (
                  ...
                  >
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      {turn.emotional_tone && (
                        <span title={turn.emotional_tone}>{TONE_INDICATOR[turn.emotional_tone] ?? "⚪"}</span>
                      )}
                      <p className="font-semibold text-sm">{turn.stakeholder_name}</p>
                      <span className="text-xs text-canvas/45">{turn.role}</span>
                      <span 
                        className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wider min-h-touch inline-flex items-center ${ACTION_COLORS[turn.action_type] ?? "bg-canvas/10 text-canvas/70"}`}
                        aria-label={`Action type: ${turn.action_type}`}
                      >
                        {turn.action_type.replace("_", " ")}
                      </span>
                      {turn.directed_at && (
                        <span className="text-xs text-canvas/75 px-2 py-1">→ {turn.directed_at}</span>
                      )}
                      {turn.coalition_with && (
                        <span className="text-xs text-accent-amber px-2 py-1">⚡ w/{turn.coalition_with}</span>
                      )}
                      {turn.leverage_gained && (
                        <span className="text-xs text-green-400 px-2 py-1">↑ leverage</span>
                      )}
                    </div>
                    <p className="text-sm leading-relaxed text-canvas/80">{turn.content}</p>
